...
template <uint8_t N>
constexpr sequence_t mask{genMask<N>()};
...
template <uint8_t N>
auto all{[]() -> std::array<sequence_t, size<N>()> {
  auto                              max = std::numeric_limits<sequence_t>::max() & mask<N>;
...
template <uint8_t N>
auto clusterSizes{[]() -> std::array<std::size_t, N - 1> {
...
// all sequences from 1 until the last integer with N set bits
template <uint8_t N>
auto allClustered{[]() -> std::array<sequence_t, size<N>()> {
  auto                              max = std::numeric_limits<sequence_t>::max() & mask<N>;
  std::array<sequence_t, size<N>()> values{};
...
template <uint8_t N>
std::string to_string(sequence_t s);
template <uint8_t N>
std::string to_set_string(sequence_t s);
}  // namespace sequence::binary
