...
template <uint8_t N>
using permutation_t = ::permutation::permutation_t<N>;
...
TEST_CASE("apply a permutation to a sequence") {
  ...
  auto max = std::numeric_limits<sequence_t>::max() & ::sequence::binary::mask<N>;
  for (sequence_t s{1}; s < max; s++) {
    REQUIRE(s == ::permutation::apply<N>(p, s));
  }

  const auto p2{perm<4>({0, 1, 3, 2})};
  const auto permuted = ::permutation::apply<4>(p2, 0b0110);
  REQUIRE(0b0101 == permuted);
}

template <uint8_t N, uint8_t K, SeqSet set_t>
void testSubsumptionFromScratch() {
}
...
TEST_CASE("incorrect permutation of outputs") {
  ...
  static_assert(N == 5);
  using Net = Network<N, K>;
  using set_t = ListNaive<N, K>;
  ...
  constexpr sequence_t mask{::sequence::binary::genMask<N>()};
  ...
  CHECK_FALSE(::permutation::subsumes<N>(p, Oa, Ob));
}
...
TEST_CASE("apply permutation") {
  ...
  const auto       p{perm<N>({1, 0, 2, 3, 4})};
  const sequence_t permuted = permutation::apply<N>(p, s);
...
TEST_CASE("find permutations given constraints") {
  ...
  std::vector<::permutation::permutation_t<N>> got{};
  ...
  const std::vector<::permutation::permutation_t<N>> wants{
      {2, 4, 3, 1, 0},
      {2, 4, 3, 0, 1},
      {3, 4, 2, 1, 0},
      {3, 4, 2, 0, 1},
...
TEST_CASE("subsumes by perfect matching on partition sets") {
  ...
  ::permutation::constraints<N>(constraints, A, B);
  REQUIRE(::permutation::valid_fast<N>(constraints));

  // the permutation (2, 0, 1, 3) should cause this to subsume
  // note that this permutation looks at the sequences
  // from left to right.
  const bool success = ::permutation::generate<N>(
      constraints,
      [&](const ::permutation::permutation_t<N> &p) {
        return ::permutation::subsumes<N>(p, A, B);
      });
      REQUIRE(success);
}